        ...
        <div className="flex flex-col items-center justify-center h-full text-center">
            ...
            <div className="mt-8 flex flex-wrap justify-center gap-3">
                {suggestions.map(text => (
                    <button key={text} onClick={() => onSuggestionClick(text)} className="bg-slate-100 hover:bg-slate-200 text-slate-700 text-sm px-4 py-2 rounded-full transition-colors">
                        {text}
                    </button>
                ))}
            </div>
...
export const Chat = () => {
    ...
    const handleSend = (text: string) => {
        ...
        setTimeout(() => {
            const aiResponse: Message = { id: (Date.now() + 1).toString(), text: `Это моковый ответ на ваш вопрос: "${userMessage.text}"`, sender: 'ai' };
    ...
    return (
        <div className="bg-white rounded-2xl border border-slate-200 flex flex-col h-[75vh]">
            ...
            {/* FIX: Новая, минималистичная форма ввода */}
