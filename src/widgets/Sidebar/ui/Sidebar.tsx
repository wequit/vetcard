...
const SidebarContent = ({ isCollapsed, onCollapseToggle, navConfig, onCloseMobile, onLogout }: {
    isCollapsed: boolean;
    ...
    return (
        <div className="flex flex-col h-full">
            ...
            <nav className="flex-grow p-3 space-y-4 overflow-y-auto">
                {navConfig.map((section) => (
                    <div key={section.title}>
                        <AnimatePresence>
                            {!isCollapsed && (
                                <motion.h3 initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="px-3 py-2 text-xs font-semibold text-slate-400 uppercase tracking-wider">
                                    {section.title}
                                </motion.h3>
                            )}
                        </AnimatePresence>
            ...
            <div className="p-3 mt-auto hidden lg:block flex-shrink-0">
                <button onClick={onCollapseToggle} className={`w-full flex items-center rounded-lg px-3 py-2.5 text-sm transition-colors text-slate-800 hover:bg-slate-100 ${isCollapsed ? 'justify-center' : 'space-x-4'}`}>
                    <div className="text-xl w-6 flex justify-center">{isCollapsed ? <FaChevronRight /> : <FaChevronLeft />}</div>
                    <AnimatePresence>{!isCollapsed && <motion.span className="whitespace-nowrap">Свернуть</motion.span>}</AnimatePresence>
                </button>
            </div>
        </div>
    );
